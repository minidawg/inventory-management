-- ═══════════════════════════════════════════════════════════════════════════════
-- Migration 0010: Vendors, Accounts Payable, and Expense category update
-- For client/samira (Sangat) — additive only, does NOT break other tenants
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ─── 2. Vendor payments table (accounts payable ledger) ──────────────────────
...
-- ─── 3. Add vendor columns to purchases (additive — old column untouched) ────
-- vendor_id: FK to vendors table (nullable for backward compat)
-- amount_paid_at_purchase: how much was paid to the vendor at stock-in time
...
-- ─── 4. Add vendor_id to overheads for vendor debt settlement payments ───────
